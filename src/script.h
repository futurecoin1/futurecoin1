...
enum {
    ...
    SCRIPT_VERIFY_NOCACHE =
        (1U << 0),  // do not store results in signature cache (but do query it)
    SCRIPT_VERIFY_NULLDUMMY = (1U << 1),  // verify dummy stack item consumed by
                                          // CHECKMULTISIG is of zero-length
...
class CScript : public std::vector<unsigned char> {
   ...
   public:
    ...
    // explicit CScript(char b) is not portable.  Use 'signed char' or 'unsigned
    // char'.
    ...
    // CScript& operator<<(char b) is not portable.  Use 'signed char' or
    // 'unsigned char'.
    ...
    CScript& operator<<(const std::vector<unsigned char>& b) {
        if (b.size() < OP_PUSHDATA1) {
            ...
            insert(end(), (unsigned char*)&nSize,
    ...
    // Called by IsStandardTx and P2SH VerifyScript (which makes it
    // consensus-critical).
    ...
    std::string ToString(bool fShort = false) const {
        ...
        while (pc < end()) {
            ...
            if (0 <= opcode && opcode <= OP_PUSHDATA4)
                str +=
                    fShort ? ValueString(vch).substr(0, 10) : ValueString(vch);
            else
...
bool EvalScript(std::vector<std::vector<unsigned char> >& stack,
                const CScript& script, const CTransaction& txTo,
...
bool SignSignature(const CKeyStore& keystore, const CScript& fromPubKey,
                   CTransaction& txTo, unsigned int nIn,
...
// Given two sets of signatures for scriptPubKey, possibly with OP_0
// placeholders,
