...
inline std::string EncodeBase58(const unsigned char* pbegin,
    ...
    // Extra zero at the end make sure bignum will interpret as a positive
    // number
...
inline bool DecodeBase58(const char* psz, std::vector<unsigned char>& vchRet) {
    ...
    int nLeadingZeros = 0;
    for (const char* p = psz; *p == pszBase58[0]; p++) nLeadingZeros++;
...
// Decode a base58-encoded string psz that includes a checksum, into byte vector
// vchRet
// returns true if decoding is successful
...
// Decode a base58-encoded string str that includes a checksum, into byte vector
// vchRet
// returns true if decoding is successful
...
class CBase58Data {
   protected:
    ...
    typedef std::vector<unsigned char,
                        zero_after_free_allocator<unsigned char> >
        vector_uchar;
...
typedef CBitcoinExtKeyBase<CExtKey, 74, CChainParams::EXT_SECRET_KEY>
    CBitcoinExtKey;
typedef CBitcoinExtKeyBase<CExtPubKey, 74, CChainParams::EXT_PUBLIC_KEY>
    CBitcoinExtPubKey;
