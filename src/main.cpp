...
#if defined(NDEBUG)
# error "FutureCoin1 cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "FutureCoin1 Signed Message:\n";
...
int64_t GetMoneySupply(const CBlockIndex* pindex)
{
    return pindex ? pindex->nMoneySupply : 0;
}
...
int64_t GetProofOfWorkReward(const CBlockIndex* pindexLast, int64_t nFees)
{
    ...
    int64_t nSupply = GetMoneySupply(pindexLast);
    int64_t nSubsidy = (POW_EMITTANCE - nSupply) >> 14;
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    int64_t nSupply = GetMoneySupply(pindexPrev);
    
    if (nSubsidy + nSupply > MAX_MONEY)
        return 0;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfWork())
    {
        int64_t nReward = GetProofOfWorkReward(pindexLast, nFees);
...
{
    RenameThread("futurecoin1-loadblk");
