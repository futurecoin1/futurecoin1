...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    const CBlockIndex* pindexLast = pindexBest ? pindexBest->pprev : NULL;
    return (uint64_t)GetProofOfWorkReward(pindexLast, 0);
}
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateFuturecoins(fGenerate, *pwalletMain);
    return Value::null;
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    const CBlockIndex* pindexLast = pindexBest ? pindexBest->pprev : NULL;
    obj.push_back(Pair("blockvalue",    (uint64_t)GetProofOfWorkReward(pindexLast, 0)));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "FutureCoin1 is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "FutureCoin1 is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "FutureCoin1 is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "FutureCoin1 is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    result.push_back(Pair("tagret", hashTarget.GetHex()));
