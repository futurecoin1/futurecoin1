...
class CAddrInfo : public CAddress {
   ...
   public:
    IMPLEMENT_SERIALIZE(CAddress *pthis = (CAddress *)(this); READWRITE(*pthis);
                        READWRITE(source); READWRITE(nLastSuccess);
                        READWRITE(nAttempts);)
 ...
 *
 * Design goals:
 *  * Keep the address tables in-memory, and asynchronously dump the entire to
 * able in peers.dat.
 ...
 *      * The actual bucket is chosen from one of these, based on the range the
...
class CAddrMan {
   ...
   protected:
    ...
    // nUnkBias determines how much to favor new addresses over tried ones
    // (min=0, max=100)
    CAddress Select_();
   ...
   public:
     ...
     *
     * vvNew is serialized, but only used if ADDRMAN_UNKOWN_BUCKET_COUNT didn't
     * change,
