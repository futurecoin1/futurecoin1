...
#define CUINT8BEGIN(a) ((const uint8_t*)&(a))

// This is needed because the foreach macro can't get over the comma in pair<t1,
// t2>
...
#define MAKE_ERROR_AND_LOG_FUNC(n)                                            \
    /*   Print to debug.log if -debug=category switch is given OR category is \
     * NULL. */                                                               \
...
inline std::string i64tostr(int64_t n) { return strprintf("%d", n); }
...
static inline uint32_t insecure_rand(void) {
    insecure_rand_Rz =
        36969 * (insecure_rand_Rz & 65535) + (insecure_rand_Rz >> 16);
    insecure_rand_Rw =
        18000 * (insecure_rand_Rw & 65535) + (insecure_rand_Rw >> 16);
...
// Use it like:
//   new boost::thread(boost::bind(&LoopForever<void (*)()>, "dumpaddr",
//   &DumpAddresses, 900000));
// or maybe:
//    boost::function<void()> f = boost::bind(&FunctionWithArg, argument);
//    threadGroup.create_thread(boost::bind(&LoopForever<boost::function<void()>
//    >, "nothing", f, milliseconds));
template <typename Callable>
void LoopForever(const char* name, Callable func, int64_t msecs) {
    std::string s = strprintf("futurecoin1-%s", name);
