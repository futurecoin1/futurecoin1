...
/** Default for -maxorphanblocksmib, maximum number of memory to keep orphan
 * blocks */
...
/** Modifier used to adjust the emittance curve, so that we hit around
  12 million coins after pow */
static const int64_t POW_EMITTANCE = 26897554 * COIN;
   ...
   public:
    ...
    std::string ToString() const {
        if (IsNull())
            ...
            return strprintf("(nFile=%u, nBlockPos=%u, nTxPos=%u)", nFile,
                             nBlockPos, nTxPos);
    }
};
...
int64_t GetMinFee(const CTransaction& tx, unsigned int nBlockSize = 1,
...
class CTransaction {
   public:
    ...
    CTransaction(int nVersion, unsigned int nTime,
                 const std::vector<CTxIn>& vin, const std::vector<CTxOut>& vout,
                 unsigned int nLockTime)
        : nVersion(nVersion),
          nTime(nTime),
          vin(vin),
    ...
    IMPLEMENT_SERIALIZE(READWRITE(this->nVersion); nVersion = this->nVersion;
                        READWRITE(nTime); READWRITE(vin); READWRITE(vout);
                        READWRITE(nLockTime);)
    ...
    /** Amount of bitcoins coming in to this transaction
       ...
       previous transactions,
        ...
        @param[in] mapInputs	Map of previous transactions that have
    ...
    bool ReadFromDisk(CDiskTxPos pos, FILE** pfileRet = NULL) {
        CAutoFile filein =
            CAutoFile(OpenBlockFile(pos.nFile, 0, pfileRet ? "rb+" : "rb"),
    ...
    std::string ToString() const {
        ...
        str += strprintf(
            "(hash=%s, nTime=%d, ver=%d, vin.size=%u, vout.size=%u, "
            "nLockTime=%d)\n",
            GetHash().ToString(), nTime, nVersion, vin.size(), vout.size(),
            nLockTime);
    ...
    /** Fetch from memory and/or disk. inputsRet keys are transaction hashes.
     ...
     @param[in] mapTestPool	List of pending changes to the transaction index
     database
     @param[in] fBlock	True if being called to add a new best-block to the
     chain
    ...
    /** Sanity check previous transactions, then, if all checks succeed,
        ...
        @param[out] mapTestPool	Keeps track of inputs that need to be
       updated on disk
    ...
    bool GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev,
                    uint64_t& nCoinAge) const;

    const CTxOut& GetOutputFor(const CTxIn& input,
                               const MapPrevTx& inputs) const;
};
...
bool IsFinalTx(const CTransaction& tx, int nBlockHeight = 0,
...
class CBlock {
   public:
    ...
    uint256 GetPoWHash() const { return gollumhash(CUINT8BEGIN(nVersion)); }
    ...
    int64_t GetMaxTransactionTime() const {
        ...
        BOOST_FOREACH (const CTransaction& tx, vtx)
            maxTransactionTime =
                std::max(maxTransactionTime, (int64_t)tx.nTime);
    ...
    static uint256 CheckMerkleBranch(uint256 hash,
        ...
        BOOST_FOREACH (const uint256& otherside, vMerkleBranch) {
            if (nIndex & 1)
                hash = Hash(BEGIN(otherside), END(otherside), BEGIN(hash),
                            END(hash));
            else
                hash = Hash(BEGIN(hash), END(hash), BEGIN(otherside),
                            END(otherside));
            nIndex >>= 1;
        }
        return hash;
    ...
    bool WriteToDisk(unsigned int& nFileRet, unsigned int& nBlockPosRet) {
        ...
        CAutoFile fileout =
            CAutoFile(AppendBlockFile(nFileRet), SER_DISK, CLIENT_VERSION);
        if (!fileout)
    ...
    std::string ToString() const {
        std::stringstream s;
        s << strprintf(
            "CBlock(hash=%s, ver=%d, hashPrevBlock=%s, hashMerkleRoot=%s, "
            "nTime=%u, nBits=%08x, nNonce=%u, vtx=%u, vchBlockSig=%s)\n",
    ...
    bool CheckBlock(bool fCheckPOW = true, bool fCheckMerkleRoot = true,
...
class CBlockIndex {
   public:
    ...
    int64_t GetPastTimeLimit() const { return GetBlockTime(); }
    ...
    std::string ToString() const {
        return strprintf(
            ...
            "nStakeModifier=%016x, hashProof=%s, prevoutStake=(%s), "
            "nStakeTime=%d merkle=%s, hashBlock=%s)",
            pprev, pnext, nFile, nBlockPos, nHeight, FormatMoney(nMint),
            FormatMoney(nMoneySupply), GeneratedStakeModifier() ? "MOD" : "-",
            GetStakeEntropyBit(), IsProofOfStake() ? "PoS" : "PoW",
            nStakeModifier, hashProof.ToString(), prevoutStake.ToString(),
            nStakeTime, hashMerkleRoot.ToString(), GetBlockHash().ToString());
    }
};
...
class CDiskBlockIndex : public CBlockIndex {
   ...
   public:
    ...
    IMPLEMENT_SERIALIZE(
        ...
        READWRITE(hashNext); READWRITE(nFile); READWRITE(nBlockPos);
        READWRITE(nHeight); READWRITE(nMint); READWRITE(nMoneySupply);
        READWRITE(nFlags); READWRITE(nStakeModifier);
        READWRITE(bnStakeModifierV2); if (IsProofOfStake()) {
        ...
        READWRITE(nNonce); READWRITE(blockHash);)

    uint256 GetBlockHash() const {
        if (fUseFastIndex && (nTime < GetAdjustedTime() - 24 * 60 * 60) &&
            blockHash != 0)
            return blockHash;
...
class CBlockLocator {
   ...
   public:
    ...
    explicit CBlockLocator(uint256 hashBlock) {
        ...
        if (mi != mapBlockIndex.end()) Set((*mi).second);
    }
    ...
    void Set(const CBlockIndex* pindex) {
        ...
        while (pindex) {
            ...
            for (int i = 0; pindex && i < nStep; i++) pindex = pindex->pprev;
