...
enum WalletFeature {
    FEATURE_BASE = 1000000,  // the earliest version new wallets supports (only
...
class CKeyPool {
   public:
    ...
    IMPLEMENT_SERIALIZE(if (!(nType & SER_GETHASH)) READWRITE(nVersion);
                        READWRITE(nTime); READWRITE(vchPubKey);)
};
...
class CWallet : public CCryptoKeyStore, public CWalletInterface {
   ...
   public:
    ...
    /** Get the wallet's activity log
        ...
        @warning Returned pointers are *only* valid within the scope of passed
       acentries
     */
    TxItems OrderedTxItems(std::list<CAccountingEntry>& acentries,
    ...
    void FixSpentCoins(int& nMismatchSpent, int64_t& nBalanceInQuestion,
...
class CWalletTx : public CMerkleTx {
   ...
   public:
    ...
    IMPLEMENT_SERIALIZE(
        ...
        if (fRead) pthis->Init(NULL); char fSpent = false;
        ...
        READWRITE(fTimeReceivedIsTxTime); READWRITE(nTimeReceived);
    ...
    bool IsSpent(unsigned int nOut) const {
        ...
        if (nOut >= vfSpent.size()) return false;
        return (!!vfSpent[nOut]);
    }

    int64_t GetDebit() const {
        if (vin.empty()) return 0;
        if (fDebitCached) return nDebitCached;
    ...
    int64_t GetCredit(bool fUseCache = true) const {
        ...
        if (fUseCache && fCreditCached) return nCreditCached;
    ...
    bool IsTrusted() const {
        ...
        for (unsigned int i = 0; i < vWorkQueue.size(); i++) {
            ...
            if (!IsFinalTx(*ptx)) return false;
            int nPDepth = ptx->GetDepthInMainChain();
            if (nPDepth >= 1) continue;
            if (nPDepth < 0) return false;
            if (!pwallet->IsFromMe(*ptx)) return false;
            ...
            BOOST_FOREACH (const CTxIn& txin, ptx->vin) {
                if (!mapPrev.count(txin.prevout.hash)) return false;
...
class COutput {
   public:
    ...
    std::string ToString() const {
        return strprintf("COutput(%s, %d, %d) [%s]", tx->GetHash().ToString(),
                         i, nDepth, FormatMoney(tx->vout[i].nValue));
    }
};
