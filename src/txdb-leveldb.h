...
class CTxDB {
   ...
   protected:
    ...
    // delete for it.
    bool ScanBatch(const CDataStream& key, std::string* value,
                   bool* deleted) const;
    ...
    bool Read(const K& key, T& value) {
        ...
        if (readFromDb) {
            leveldb::Status status =
                pdb->Get(leveldb::ReadOptions(), ssKey.str(), &strValue);
            if (!status.ok()) {
    ...
    bool Write(const K& key, const T& value) {
        ...
        leveldb::Status status =
            pdb->Put(leveldb::WriteOptions(), ssKey.str(), ssValue.str());
        if (!status.ok()) {
    ...
    bool Erase(const K& key) {
        ...
        leveldb::Status status =
            pdb->Delete(leveldb::WriteOptions(), ssKey.str());
    ...
    bool Exists(const K& key) {
        ...
        leveldb::Status status =
            pdb->Get(leveldb::ReadOptions(), ssKey.str(), &unused);
