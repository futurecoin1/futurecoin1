...
CBlock *CreateNewBlock(CReserveKey &reservekey, bool fProofOfStake,
    ...
    unsigned int nBlockMaxSize =
        GetArg("-blockmaxsize", MAX_BLOCK_SIZE_GEN / 2);
    ...
    if (mapArgs.count("-mintxfee")) ParseMoney(mapArgs["-mintxfee"], nMinTxFee);
    ...
    int64_t nFees = 0;
    {
        ...
        for (map<uint256, CTransaction>::iterator mi = mempool.mapTx.begin();
            ...
            // client
            // code, because the
            // client code rounds up the size to the nearest 1K. That's good,
            // because
            // it gives an
        ...
        if (!fProofOfStake)
            pblock->vtx[0].vout[0].nValue =
                GetProofOfWorkReward(pindexPrev ? pindexPrev->pprev : NULL, nFees);;

        if (pFees) *pFees = nFees;
...
bool CheckStake(CBlock *pblock, CWallet &wallet) {
    ...
    if (!CheckProofOfStake(mapBlockIndex[pblock->hashPrevBlock], pblock->vtx[1],
...
void ThreadStakeMiner(CWallet *pwallet) {
    ...
    RenameThread("futurecoin1-miner");
...
void static FuturecoinMiner(CWallet *pwallet) {
    printf("FuturecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    try {
        while (true) {
            ...
            // Create new block
            //
            uint32_t nTransactionsUpdatedLast =
            ...
            printf(
                "Running FuturecoinMiner with %lu transactions in block (%u "
                "bytes)\n",
                pblock->vtx.size(),
                ::GetSerializeSize(*pblock, SER_NETWORK, PROTOCOL_VERSION));

            unsigned int &nBlockBits = pblock->nBits;
                    ...
                    static CCriticalSection cs;
                    {
                        ...
                        if (GetTimeMillis() - nHPSTimerStart > 4000) {
                            ...
                            if (GetTime() - nLogTime > 30 * 60) {
                                nLogTime = GetTime();
                                printf("hashmeter %6.0f Futurecoin2/s\n",
        ...
        printf("FuturecoinMiner terminated\n");
        throw;
    }
}

void GenerateFuturecoins(bool fGenerate, CWallet &pwallet) {
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FuturecoinMiner, &pwallet));
}
